import { Platform } from '../../shared/common.interface.js';
import {
  CommandArgsMap,
  CommandName,
  IBuildCommandArgs,
...
export class Core extends MessagesManager {
  // Command builders keyed by command name; each returns the shell string for
  // its platform. Initialized empty and populated in the constructor via the
  // build*Commands() helpers, hence the `as any` seed.
  ...
  constructor() {
    ...
    // Populate both command tables up front so getCommand() can dispatch by platform.
