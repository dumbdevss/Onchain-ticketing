...
import { OutputMessage, Platform } from '../../shared/common.interface.js';
import { IContractInterface } from '../core/contract.interface.js';
import { ContractParser } from '../core/contractParser.js';
import { Core } from '../core/core.js';
import {
  IConstructorArg,
  IDeployContractArgs,
} from './deployContract.interface.js';
import { IGetContractMethodsArgs } from './getContractMethods.interface.js';
...
export class Soroban extends Core {
  ...
  constructor(serverUrl: string) {
    ...
    // Infer the target network from the RPC URL, defaulting to public/mainnet.
    ...
    return new Promise((resolve) => {
      ...
      exec(command, (error, stdout, stderr) => {
        if (error) {
          ...
          // Normalize non-Windows "path not found" style failures to a single
          // consistent message, but preserve the specific missing-Cargo.toml hint.
          if (
  ...
  /** Locate all compiled `.wasm` artifacts in the release output directory. */
  ...
  private async optimizeWasmFile(
    ...
    return new Promise((resolve) => {
      // On Windows the optimize command needs the full release path; on other
      // platforms the bare file name (already absolute from `find`) is enough.
      const wasmPath =
        this.platform === Platform.WINDOWS
  ...
  /**
   * Extract the deployed contract address from CLI output. Stellar contract
   * addresses are the single line beginning with `C`, so the first non-empty
   * such line is treated as the address.
   */
  ...
  /**
   * Guard that fails deployment early if the contract source declares a
   * constructor but the caller supplied no constructor arguments.
   */
  ...
  /**
   * Determine whether the contract defines a constructor by scanning its
   * source, returning both a boolean result and any parsed argument list.
   */
  ...
  /**
   * Recursively walk the contract's source tree looking for a `__constructor`
   * definition. Only descends into directories matching the package name or a
   * `contracts` folder to avoid scanning unrelated files. Returns the parsed
   * constructor arguments from the first matching file found.
   */
  private readContractArgs(
    ...
    // Start from the workspace root relative to the compiled wasm path when no
    // explicit directory is provided (recursion supplies `currentPath`).
    const srcDir =
  ...
  private extractConstructorArgsFromFile(
    ...
    // Soroban constructors are declared as `pub fn __constructor(...)`.
  ...
  private parseConstructorArgs(args: string): IConstructorArg[] {
    return (
      args
        .split(',')
        // Drop the first parameter (the implicit `env: Env`), keeping user args.
        .slice(1)
  ...
  /** Serialize constructor args into `--name value` flags for the CLI. */
  ...
  /** Cargo package names are snake_case; directory names use kebab-case. */
